...
fn wire_resizer_impl(
    ...
    percent: impl Wire2Api<u32> + UnwindSafe,
) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap::<_, _, _, Vec<u8>, _>(
        ...
        move || {
            let api_image = image.wire2api();
            let api_width = width.wire2api();
            let api_height = height.wire2api();
            let api_percent = percent.wire2api();
            move |task_callback| {
                Result::<_, ()>::Ok(resizer(api_image, api_width, api_height, api_percent))
            }
        },
    )
}
